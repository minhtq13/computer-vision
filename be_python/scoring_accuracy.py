...
import json
from tool import generate_output, get_parameter_number_anwser, remove_elements_info, remove_elements_answer, \
    remove_elements_marker
from tool import get_class_answer, get_class_info, get_coordinates, get_coordinates_info, get_remainder, orient_image_by_angle, get_class_marker
...
def get_marker(image, model, filename, exam_class_code):
    try:
        ...
        return output, orientation, maybe_wrong_marker
...
def predictAnswer(img, filename, model, index, number_answer):
    ...
    for i, answer in enumerate(list_label):
        if index == get_parameter_number_anwser(number_answer) and i == get_remainder(number_answer):
            break
        ...
        if conf < threshold_warning:
            maybe_wrong_answer.append(f'Nhãn "{get_class_answer(class_answer)}" câu {index * 20 + i + 1}, ảnh {filename}, xác suất: {conf}')
        for char in str(get_class_answer(class_answer)):
            point1, point2, point3, point4 = get_coordinates(x1, y1, x2, y2, char)
            # Vẽ hình chữ nhật lên các nhãn không khoanh chỉ khi conf < threshold_warning
            if char == "x":
                if conf < threshold_warning:
                ...
                cv2.rectangle(img, (point1, point2), (point3, point4),
                            green_color if conf > threshold_warning else warning_color,
                            1 if conf > threshold_warning else 2)
...
def predict_info(img, model, filename):
    results = model.predict(img)
    data = results[0].boxes.data
    ...
    for i, info in enumerate(list_label):
        ...
        if conf < threshold_warning:
            maybe_wrong_info.append(
                f'Nhãn thứ {i} từ trái sang phải: "{get_class_info(class_info)}", xác suất: {conf}, ảnh {filename}')
        ...
        cv2.putText(img, str(get_class_info(class_info)) if conf > threshold_warning else str(f"{get_class_info(class_info)}-{conf}"),
                    ...
                    0.4 if conf > threshold_warning else 0.5, blue_color if conf > threshold_warning else warning_color,
                    1, cv2.LINE_AA, )
    if numberClassRecognition > 5:
        class_code = "".join(
            [dict_info[str(i)] if str(i) not in dict_info or dict_info[str(i)] != "unchoice" else "x" for i in
             range(1, 7)])
        ...
        img_resize = cv2.resize(img, (506, 500), interpolation=cv2.INTER_AREA)
    elif numberClassRecognition <= 5:
        result_info = {"class_code": "", "student_code": "", "exam_code": ""}
        img_resize = cv2.resize(img, (506, 500), interpolation=cv2.INTER_AREA)

    return result_info, img_resize, numberClassRecognition, maybe_wrong_info


# ============================================ MERGE IMAGES =======================================
def merge_images(exam_class_code, filename, coord_array, array_img_graft, background_image, imgInfo):
    ...
    handled_scored_img = f"{SHARED_DATA_DIR}/AnsweredSheets/{exam_class_code}/HandledSheets/handled_{filename_cut}.jpg"
    # relative path from server
    handled_scored_img_filename = f"handled_{filename_cut}.jpg"
    cv2.imwrite(handled_scored_img, background_image * 255)
    return handled_scored_img, handled_scored_img_filename


def score(exam_class_code: str, selected_images: list, number_answer: int ):
    print("SCORING ACCURACY number_answer", number_answer)
    global maybe_wrong_answer_array, maybe_wrong_info, maybe_wrong_marker
    # ========================== Đo thời gian ====================================
    # start_time = time.time()
    # ===================== Khai báo và load model ==============================
    pWeight_marker = "./Model/marker_11.pt"
    pWeight_answer = "./Model/answer_m_11.pt"
    pWeight_info = "./Model/info_m_11.pt"
    model_info = YOLO(pWeight_info)
    model_marker = YOLO(pWeight_marker)
    model_answer = YOLO(pWeight_answer)

    print("scoring_accuracy.py: exam_class_code:", exam_class_code)
    ...
    folder_path = f"{SHARED_DATA_DIR}/AnsweredSheets/{exam_class_code}"
    folder_path_handle = f"{SHARED_DATA_DIR}/AnsweredSheets/{exam_class_code}/HandledSheets"
    folder_scored_path = f"{SHARED_DATA_DIR}/AnsweredSheets/{exam_class_code}/ScoredSheets"
    folder_maybe_wrong = f"{SHARED_DATA_DIR}/AnsweredSheets/{exam_class_code}/MayBeWrong"
    ...
    if not os.path.exists(folder_maybe_wrong):
        try:
            ...
            # ================================= Chương trình chính=================================
    file_names = os.listdir(folder_path) if "ALL" in selected_images else selected_images
    file_names.sort()
    for filename in file_names:
        if filename.lower().endswith((".jpg", ".jpeg", ".png")):
            image_path = os.path.join(folder_path, filename)
            image = cv2.imread(image_path)
            document, oritentation, maybe_wrong_marker = get_marker(image, model_marker, filename, exam_class_code)
            if document is None:
                continue
            if oritentation == 'bl':
            ...
            result_info, imgResize, numberClassRecognition, maybe_wrong_info = predict_info(img=img_resize, model=model_info,
                                                                                           filename=filename)
            # =================================== Lấy đáp án ==============================
            result_answer, size_array, coord_array = crop_image_answer(cv2.convertScaleAbs(document * 255),
                                                                       number_answer)
            list_answer = []
            array_img_graft = []
            for i, answer in enumerate(result_answer):
                selected_answer, img_graft, maybe_wrong_answer = predictAnswer(img=answer, filename=filename,
                                                                               model=model_answer, index=i,
            ...
            # ============================= Ghép ảnh =====================================
            handled_scored_img, handled_scored_filename = merge_images(exam_class_code, filename, coord_array,
                                                                       array_img_graft,
            ...
            result["handledScoredFileName"] = handled_scored_filename
